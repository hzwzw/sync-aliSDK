...
TopBatch.prototype.execute = function (reqs, publicReq=null) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    that.request(reqs, publicReq, function (err, result) {
      ...
      const response = results.map((res, index) => {
...
TopBatch.prototype.request = function (reqs, publicReq=null, callback) {
  ...
  function linePayload (obj) {
    if (obj.sessionInfo) {
      const now = Date.now();
      const thirdExpiredTime = obj.seessionInfo.sessionExpireTime - 5000;
      if (now > thirdExpiredTime) {
        let err = new Error('aengine pre check, auth seesion expired');
        err.code = 27;
        return callback(err);
      }
      obj.session = obj.sessionInfo.session;
