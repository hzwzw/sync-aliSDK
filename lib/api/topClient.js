 ...
 * @public
 */
TopClient.prototype.request = function (type, params, callback, retry = 2) {
  ...
  if (skipAuth) {
    ...
    const now = Date.now();
    const thirdExpiredTime = params.sessionInfo.sessionExpireTime;
  ...
  request.end(function(response){
    if(response.statusCode === 200){
      ...
      if (errRes) {
        if (errRes.code === 15 && retry > 0) {
          console.log(`retry retry ${retry}`, backupParams);
          that.request(type, backupParams, callback, retry - 1);
          return;
        }
